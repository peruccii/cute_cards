...
describe('FIREBASE UPLOAD IMAGES TEST', () => {
        ...
        Firebase,
        {
          ...
          useValue: {
            get: jest.fn().mockImplementation((key: string) => {
              if (key === 'STORAGE_BUCKET_NAME') {
                // verifica se a key usada em "UploadImagesToFirebase" coincide com essa usado no mock
  ...
  it('should be able to upload image(s) to firebase storage', async () => {
    ...
    const imagesUrls = await uploadImage.uploadImages(
      [mockFile, mockFile_2],
      invite.email.value,
    );
